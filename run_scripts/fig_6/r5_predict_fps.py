...
res_folder = Path("results/2023_05_04_forward_analysis")
...
for subdir in Path(res_folder).glob("*"):
    ...
    save_name = subdir / f"fold_{fold_num}_morgan4096_preds.p"
    cmd_1 = f"python run_scripts/predict_forward_ffn.py --checkpoint {ckpt_file} --batch-size 64 --num-workers 16 --dataset-name {dataset} --save-tuples --save-name {save_name} --gpu" 
    print(cmd_1)
